...
export class DocumentService {
  ...
  static async getStudentDocuments(studentId: number) {
    ...
    const groupedDocuments = {
      photo: documents.filter((doc: Document) => doc.type === 'PHOTO'),
      transcript: documents.filter((doc: Document) => doc.type === 'TRANSCRIPT'),
      certificate: documents.filter((doc: Document) => doc.type === 'CERTIFICATE'),
      supporting: documents.filter((doc: Document) => doc.type === 'SUPPORTING')
    };
