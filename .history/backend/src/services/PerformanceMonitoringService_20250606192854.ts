...
class PerformanceMonitoringService {
  ...
  getPerformanceAnalytics() {
    ...
    const slowestQuery = recentMetrics.length > 0 
      ? recentMetrics.reduce((slowest: QueryMetrics | undefined, current) => 
          !slowest || current.duration > slowest.duration ? current : slowest, undefined as QueryMetrics | undefined)
      : null;
