  ...
  static async sendPasswordResetEmail(
    ...
    try {
      ...
      const { data, error } = await resend.emails.send({
        from: 'EAU Credential System <onboarding@resend.dev>', // Using Resend's test domain
        to: [email],
        subject: 'Password Reset Request - EAU Credential System',
        html: this.getPasswordResetTemplate(verificationCode, userName),
        text: `Your password reset verification code is: ${verificationCode}. This code expires in 15 minutes.`
      });
