...
export class DocumentService {
  ...
  static async getStudentDocuments(studentId: number) {
    ...
    const groupedDocuments = {
      photo: documents.filter((doc: { type: string }) => doc.type === 'photo'),
      transcript: documents.filter((doc: { type: string }) => doc.type === 'transcript'),
      certificate: documents.filter((doc: { type: string }) => doc.type === 'certificate'),
      supporting: documents.filter((doc: { type: string }) => doc.type === 'supporting')
    };
