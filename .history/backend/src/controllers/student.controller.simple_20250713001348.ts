...
export class SimpleStudentController {
   ...
   * Target: Fast student updates with validation and cache invalidation
   */
  async updateStudent(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
      ...
      // Use partial schema for updates (make all fields optional)
      ...
      // Check for duplicate registration/certificate IDs (excluding current student)
      ...
      // Update student with minimal relation loading for faster response
      ...
      // Clear cache for updated student
      studentDetailsCache.delete(studentId);

      console.timeEnd('⚡ Student Update Performance');
      console.log(`⚡ Updated student ${updatedStudent.registrationId}`);
