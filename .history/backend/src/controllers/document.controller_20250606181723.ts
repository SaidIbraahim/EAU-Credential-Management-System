...
export class DocumentController {
  ...
  static async bulkUploadDocuments(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Organize files by their names (which should contain registration ID info)
      const organizedFiles: {[registrationId: string]: {[docType: string]: Express.Multer.File[]}} = {};
      
      // Process each uploaded file and organize by registration ID and document type
      for (const file of files) {
        try {
          // Extract registration ID from filename
          // Assuming filename format: registrationId.extension (e.g., GRW-BCS-2007.pdf)
          const fileName = file.originalname || (file as any).name || 'unknown';
          const fileNameWithoutExt = fileName.split('.')[0];
          
          // For now, let's try to extract registration ID from the filename
          // This is a simplified approach - we might need to enhance this logic
          const registrationId = fileNameWithoutExt;
          
          // Try to determine document type from file metadata or use 'supporting' as default
          const docType = 'supporting'; // We'll need to enhance this logic
