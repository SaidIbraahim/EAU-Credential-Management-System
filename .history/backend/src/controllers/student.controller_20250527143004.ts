...
export const StudentController = {
    ...
    try {
      ...
      if (existingStudent) {
        return res.status(400).json({
          error: 'Student with this registration ID, student ID, or certificate ID already exists'
        });
      }
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      if (validatedData.registrationId || validatedData.studentId || validatedData.certificateId) {
        ...
        if (duplicate) {
          return res.status(400).json({
            error: 'Another student with the provided registration ID, student ID, or certificate ID already exists'
          });
        }
      }
  ...
  async bulkCreate(req: Request, res: Response) {
    try {
      ...
      if (existingStudents.length > 0) {
        return res.status(400).json({
          error: 'Some students with provided registration IDs, student IDs, or certificate IDs already exist',
          conflicts: existingStudents
        });
      }
