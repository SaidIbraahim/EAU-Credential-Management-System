...
export class DocumentController {
  ...
  static async downloadDocument(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Check if we should use direct presigned URLs or proxy through backend
      const useProxy = process.env.CLOUD_STORAGE_USE_PROXY === 'true';
      
      if (useProxy) {
        // Proxy method: Stream file through backend
        ...
        // Presigned URL method (requires proper R2 bucket configuration)
  ...
  // New route for direct file streaming (proxy method)
  static async streamDocument(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Set response headers for direct viewing
      res.setHeader('Content-Type', response.ContentType || 'application/octet-stream');
      res.setHeader('Content-Length', response.ContentLength || 0);
      res.setHeader('Cache-Control', 'private, max-age=3600');
