...
export class DocumentController {
  ...
  static async bulkUploadDocuments(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!organizationData) {
        throw new AppError('Organization data is required for bulk upload', 400);
      }

      // Parse organization data (JSON string from frontend)
      let organizedFiles: {[registrationId: string]: {[docType: string]: Express.Multer.File[]}};
      try {
        organizedFiles = JSON.parse(organizationData);
      } catch (error) {
        throw new AppError('Invalid organization data format', 400);
      }
