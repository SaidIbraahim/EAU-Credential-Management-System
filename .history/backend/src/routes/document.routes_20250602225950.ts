...
import { testR2Connection, listR2Documents } from '../utils/testR2';
...
// Debug route to test R2 connection (remove in production)
router.get('/debug/r2-test', async (_req, res) => {
  try {
    const isConnected = await testR2Connection();
    await listR2Documents();
    res.json({ 
      r2Connected: isConnected,
      message: 'Check server logs for detailed R2 status'
    });
  } catch (error) {
    res.status(500).json({ error: 'R2 test failed', details: error });
  }
});
