      ...
      if (zipFile) {
        ...
        if (allZipErrors.length > 0) {
          ...
          if (zipValidation.invalidRegistrationIds.length > 0) {
            toast.warning(`ZIP processed with ${zipValidation.invalidRegistrationIds.length} unregistered students. Documents for unregistered students will be skipped.`);
          } else {
        ...
        if (!csvFile) {
          ...
          if (validRegIds === 0) {
            ...
            toast.info(`Ready to upload documents for ${validRegIds}/${totalRegIds} registered students. Click "Confirm Import" to proceed.`);
          }
        }
      }
