...
const ImportActionButtons = ({
  ...
  return (
    <div className="mt-4">
      {/* Import Summary */}
      {(totalStudents > 0 || documentCount > 0) && (
          ...
          <div className="text-sm text-blue-700">
            {totalStudents > 0 && (
              <>
                ...
                <p>• New students to import: <span className="font-medium text-green-600">{willImportStudents}</span></p>
              </>
            )}
            {documentCount > 0 && (
              <p>• Documents to upload: <span className="font-medium text-purple-600">{documentCount}</span></p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex justify-end">
        ...
        >
          {isLoading ? 'Processing...' : 
            hasStudentsToImport && hasDocumentsToImport ? `Import ${willImportStudents} Students + ${documentCount} Documents` :
            hasStudentsToImport ? `Import ${willImportStudents} Student${willImportStudents !== 1 ? 's' : ''}` :
            hasDocumentsToImport ? `Upload ${documentCount} Documents` :
            'Nothing to Import'
          }
        </Button>
      </div>
