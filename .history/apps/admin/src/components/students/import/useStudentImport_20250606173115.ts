...
import { parseCSV, processZipFile, validateStudents, validateZipRegistrationIds } from "@/utils/fileUtils";
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (zipFile) {
        ...
        toast.info(`Processing ${zipFile.name} (${(zipFile.size / 1024 / 1024).toFixed(1)}MB)...`, {
          duration: 2000
        });
        ...
        console.log(`⚡ ZIP processed in ${processingTime}s`);
        
        // Validate registration IDs in ZIP against existing students
        const zipValidation = validateZipRegistrationIds(zipResult.organizedFiles, allStudentsForValidation);
        ...
        // Combine ZIP processing errors with validation warnings
        const allZipErrors = [...zipResult.errors, ...zipValidation.warnings];
        
        if (allZipErrors.length > 0) {
          // Add ZIP errors to the errors state so they show in the UI
          setErrors(prev => [...prev, ...allZipErrors]);
          
          if (zipValidation.invalidRegistrationIds.length > 0) {
            toast.warning(`ZIP processed in ${processingTime}s with ${zipValidation.invalidRegistrationIds.length} unregistered students. Documents for unregistered students will be skipped.`);
          } else {
            toast.warning(`ZIP processed in ${processingTime}s with ${zipResult.errors.length} warnings. Check details below.`);
          }
        } else {
          toast.success(`⚡ ZIP processed successfully in ${processingTime}s: ${zipResult.totalFiles} documents for ${Object.keys(zipResult.organizedFiles).length} students`);
        }
        
        // If only ZIP file (no CSV), we're doing document-only import
        if (!csvFile) {
          const validRegIds = zipValidation.validRegistrationIds.length;
          const totalRegIds = Object.keys(zipResult.organizedFiles).length;
          
          if (validRegIds === 0) {
            toast.error("No valid registration IDs found in ZIP file. All students must be registered before uploading documents.");
          } else {
            toast.info(`Ready to upload ${zipResult.totalFiles} documents for ${validRegIds}/${totalRegIds} registered students. Click "Upload Documents" to proceed.`);
          }
        }
      }
