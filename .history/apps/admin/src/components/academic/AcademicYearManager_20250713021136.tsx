...
const AcademicYearManager = ({ preloadedAcademicYears }: AcademicYearManagerProps) => {
  ...
  const {
    ...
    invalidateCache: invalidateAcademicYearsCache
  } = useOptimizedData(
  ...
  const onSubmit = async (values: AcademicYearFormValues) => {
    try {
      ...
      // 🚀 COMPREHENSIVE CACHE INVALIDATION (CRITICAL FIX)
      // Clear ALL academic-related caches to ensure real-time updates
      invalidateAcademicCache();
      
      // Also invalidate local cache and refetch
      invalidateAcademicYearsCache();
      refetchAcademicYears();
      
      console.log('✅ Academic year cache invalidated - new items should appear immediately');
  ...
  const deleteAcademicYear = async (academicYearId: number) => {
    try {
      ...
      console.log('✅ Academic year deleted - cache invalidated for real-time updates');
