...
export const processZipFile = async (file: File): Promise<{
  ...
  try {
    ...
    console.log('📁 ZIP Structure Analysis:', {
      ...
      rootFiles: Object.keys(zipContent.files).filter(name => !name.includes('/'))
    });
    ...
    // Define expected folder structure and file types
    ...
    for (const [filePath, zipEntry] of Object.entries(zipContent.files)) {
      ...
      // Parse file path structure: Expected formats:
      // 1. STUDENT_ID/DOCUMENT_TYPE/filename.ext
      // 2. STUDENT_ID/filename.ext (auto-detect type)
      // 3. DOCUMENT_TYPE/STUDENT_ID_filename.ext
      ...
      if (pathParts.length === 3) {
        // Format: STUDENT_ID/DOCUMENT_TYPE/filename.ext
        registrationId = pathParts[0];
        docType = pathParts[1].toLowerCase();
        fileName = pathParts[2];
      } else if (pathParts.length === 2) {
        // Format: STUDENT_ID/filename.ext
        registrationId = pathParts[0];
        fileName = pathParts[1];
        // Auto-detect document type from filename
        docType = detectDocumentType(fileName);
      } else if (pathParts.length === 1) {
        // Format: STUDENT_ID_DOCTYPE_filename.ext or similar
        const fileNameLower = filePath.toLowerCase();
        const match = fileNameLower.match(/^([^_]+)_([^_]+)_(.+)$/);
        if (match) {
          registrationId = match[1];
          docType = match[2];
          fileName = match[3];
        } else {
          errors.push(`Invalid file path structure: ${filePath}. Expected formats: STUDENT_ID/DOCUMENT_TYPE/filename.ext or STUDENT_ID/filename.ext`);
          continue;
        }
      } else {
        errors.push(`Invalid file path structure: ${filePath}. Too many nested folders.`);
        continue;
      }
      
      // Validate document type
      if (!DOCUMENT_TYPES[docType as keyof typeof DOCUMENT_TYPES]) {
        errors.push(`Invalid document type "${docType}" for file: ${filePath}. Valid types: ${Object.keys(DOCUMENT_TYPES).join(', ')}`);
        continue;
      }
      
      // Validate file extension
      const fileExt = fileName.toLowerCase().substring(fileName.lastIndexOf('.'));
      const allowedExts = DOCUMENT_TYPES[docType as keyof typeof DOCUMENT_TYPES].extensions;
      if (!allowedExts.includes(fileExt)) {
        errors.push(`Invalid file extension "${fileExt}" for ${docType} document: ${filePath}. Allowed: ${allowedExts.join(', ')}`);
        continue;
      }
      ...
      try {
        ...
        // Validate file size
        const maxSize = DOCUMENT_TYPES[docType as keyof typeof DOCUMENT_TYPES].maxSize;
        if (fileSize > maxSize) {
          errors.push(`File too large: ${filePath} (${(fileSize / 1024 / 1024).toFixed(2)}MB). Max size for ${docType}: ${(maxSize / 1024 / 1024)}MB`);
          continue;
        }
        ...
        console.log(`✅ Processed: ${registrationId}/${docType}/${fileName}`);
        
      } catch (extractError) {
        errors.push(`Failed to extract file: ${filePath}. Error: ${extractError instanceof Error ? extractError.message : 'Unknown error'}`);
      }
    }
    
    // Generate summary
    const studentCount = Object.keys(organizedFiles).length;
    const summary = `Processed ${totalFiles} files for ${studentCount} students. ${errors.length} errors found.`;
    
    console.log('📊 ZIP Processing Results:', {
      totalFiles,
      studentCount,
      errorsCount: errors.length,
      organizedFiles: Object.keys(organizedFiles).map(regId => ({
...
function detectDocumentType(fileName: string): string {
  ...
  if (fileNameLower.includes('photo') || fileNameLower.includes('picture') || fileNameLower.includes('img')) {
    return 'photo';
  } else if (fileNameLower.includes('transcript') || fileNameLower.includes('grade')) {
    return 'transcript';
  } else if (fileNameLower.includes('certificate') || fileNameLower.includes('cert') || fileNameLower.includes('diploma')) {
    return 'certificate';
  } else {
    return 'supporting';
  }
