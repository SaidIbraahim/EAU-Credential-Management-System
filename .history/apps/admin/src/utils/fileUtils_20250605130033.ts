...
const findEntityIdByName = (entities: any[], name: string, entityType: string): number => {
  ...
  console.log(`📋 Available ${entityType}s:`, entities.slice(0, 3).map(e => ({ id: e?.id, name: e?.name })));
  ...
  const entity = entities.find(item => {
    ...
    try {
      ...
      console.error(`❌ Error comparing ${entityType} names:`, { item: item.name, name, error });
      return false;
    }
  });
  ...
  if (!entity) {
    console.error(`❌ ${entityType} not found:`, { 
      searchName: name, 
      availableNames: entities.map(e => e?.name).filter(Boolean) 
    });
    throw new Error(`${entityType} "${name}" not found. Available options: ${entities.map(e => e?.name).filter(Boolean).join(', ')}`);
  }
  
  console.log(`✅ Found ${entityType}:`, { id: entity.id, name: entity.name });
  return entity.id;
};
