...
export const processZipFile = async (file: File): Promise<{
  ...
  try {
    ...
    for (const [filePath, zipEntry] of Object.entries(zipContent.files)) {
      ...
      // Parse file path: Expected format: DOCUMENT_TYPE/REGISTRATION_ID.extension
      const pathParts = filePath.split('/');
      
      if (pathParts.length !== 2) {
        errors.push(`Invalid file structure: ${filePath} (${pathParts.length} parts). Expected format: DOCUMENT_TYPE/REGISTRATION_ID.extension. Parts: [${pathParts.join(', ')}]`);
        continue;
      }
      
      const docTypeFolder = pathParts[0].toLowerCase();
