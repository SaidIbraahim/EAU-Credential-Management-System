...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    const shouldShowAutoToast = !(
      ...
      error.config?.url?.includes('/auth/logout') // Don't auto-toast logout errors - handled in logout function
    );
    ...
    if (error.response?.status === 401) {
      // Don't auto-redirect if this is a login or logout attempt
      ...
      if (errorMessage && errorMessage.includes('deactivated')) {
        // Don't auto-redirect if this is a login or logout attempt - let login page handle it
        if (!error.config?.url?.includes('/auth/login') && !error.config?.url?.includes('/auth/logout')) {
          toast.error(errorMessage);
          localStorage.removeItem('auth_token');
