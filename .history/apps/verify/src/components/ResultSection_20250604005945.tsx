...
const DataRow: React.FC<DataRowProps> = ({ icon, label, value, isHighlighted, statusValue }) => {
  const getStatusColor = (status?: string) => {
    if (!status) return '';
    return status === 'CLEARED' ? 'text-green-600 font-medium' : 'text-yellow-600 font-medium';
  };
  ...
  return (
    <tr className="border-b border-gray-100 hover:bg-gray-50">
      ...
      <td className="py-3 px-4 border-r border-gray-200">
        <span className={`${isHighlighted ? 'text-[#d30000] font-bold' : ''} ${getStatusColor(statusValue)}`}>
          {value}
        </span>
      </td>
    </tr>
