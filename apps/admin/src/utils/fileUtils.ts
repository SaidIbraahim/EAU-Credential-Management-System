...
export const processZipFile = async (file: File): Promise<{
  ...
  try {
    ...
    // Read ZIP file with optimized settings
    const zipContent = await zip.loadAsync(file, {
      checkCRC32: false, // Skip CRC check for speed
      optimizedBinaryString: true,
      createFolders: false // Don't create empty folders
    });
    
    console.log('📁 ZIP Structure Analysis (Fast Mode):', {
      totalEntries: Object.keys(zipContent.files).length,
      estimatedProcessingTime: `${Math.ceil(Object.keys(zipContent.files).length / 10)}s`
    });
    ...
    // Pre-compiled document type configurations for faster lookup
    ...
    // Pre-compiled regex for faster folder matching
    const DOC_TYPE_PATTERNS = {
      photo: /^(photo|photos|picture|pictures)$/i,
      certificate: /^(certificate|certificates|cert|certs)$/i,
      transcript: /^(transcript|transcripts|grade|grades)$/i,
      supporting: /^(supporting|support|other|misc)$/i
    };
    
    // Filter and batch files for processing
    const validFiles = Object.entries(zipContent.files).filter(([filePath, zipEntry]) => 
      !zipEntry.dir && filePath.includes('/') && !filePath.startsWith('__MACOSX')
    );
    
    console.log(`🚀 Processing ${validFiles.length} files in optimized batches...`);
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      const batchPromises = batch.map(async ([filePath, zipEntry]) => {
        try {
          // Parse file path with optimized logic
          ...
          if (!docType) {
            return { error: `No valid document type folder found in path: ${filePath}` };
          }
          
          // Fast file validation
          const lastDotIndex = fileName.lastIndexOf('.');
          if (lastDotIndex === -1) {
            return { error: `Invalid filename format: ${fileName}. Missing extension.` };
          }
          ...
          if (!registrationId || registrationId.length < 3) {
            return { error: `Invalid registration ID: "${registrationId}" in file ${fileName}` };
          }
          
          const docConfig = DOCUMENT_TYPES[docType as keyof typeof DOCUMENT_TYPES];
          if (!docConfig.extensions.has(fileExtension)) {
            return { error: `Invalid file extension "${fileExtension}" for ${docType}: ${fileName}` };
          }
          
          // Extract file with optimized settings
          const fileData = await zipEntry.async('arraybuffer');
          const fileSize = fileData.byteLength;
          
          if (fileSize > docConfig.maxSize) {
            return { error: `File too large: ${fileName} (${(fileSize / 1024 / 1024).toFixed(1)}MB)` };
          }
          
          // Create File object efficiently
          ...
          return { error: `Failed to extract file: ${filePath}. ${extractError instanceof Error ? extractError.message : 'Unknown error'}` };
        }
      });
      ...
      for (const result of batchResults) {
        if (result.error) {
          ...
          const { registrationId, docType, file, fileName } = result;
    ...
    let summary = `Fast processed ${totalFiles} files for ${studentCount} students across ${docTypesFound.length} document types.`;
    if (errors.length > 0) {
    ...
    console.log('🎯 Optimized ZIP Processing Results:', {
      totalFiles,
      studentCount,
      documentTypeStats,
