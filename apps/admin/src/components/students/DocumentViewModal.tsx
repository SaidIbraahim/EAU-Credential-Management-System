...
const DocumentViewModal = ({
  ...
  const handleImageError = useCallback((doc: Document) => {
    console.error('Image load failed for document:', doc.fileName);
    // Don't show toast here as it creates noise - the user can see the image failed to load
  }, []);
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            {isImageFile(viewingDocument) ? (
              <div className="flex-1 flex items-center justify-center bg-gray-100 rounded-md overflow-auto">
                <img 
                  src={viewingDocument.presignedUrl || viewingDocument.fileUrl} 
                  alt={viewingDocument.fileName}
                  className="max-w-full max-h-full object-contain"
                  onError={() => handleImageError(viewingDocument)}
                />
              </div>
            ) : (
              <div className="flex-1 flex items-center justify-center bg-gray-100 p-8 rounded-md">
                <div className="text-center">
                  ...
                  <Button onClick={() => window.open(viewingDocument.presignedUrl || viewingDocument.fileUrl, '_blank')}>
