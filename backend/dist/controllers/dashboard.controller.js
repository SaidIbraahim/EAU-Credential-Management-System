...
class DashboardController {
    static async getStats(_req, res) {
        try {
            ...
            const [basicStats, departmentStats, gradeDistribution, monthlyTrends] = await Promise.all([
        ...
        `
            ...
            const deptList = departmentStats;
            const bestDepartment = deptList.length > 0
                ? deptList.reduce((best, dept) => Number(dept.average_gpa) > Number(best.average_gpa) ? dept : best)
            ...
            const statistics = {
                ...
                trends: {
                    monthlyRegistrations: monthlyTrends.map(trend => ({
                        month: trend.month,
    ...
    static async getQuickStats(_req, res) {
        try {
            ...
            const quickStatsResult = await prisma_1.prisma.$queryRaw `
        SELECT 
          COUNT(*)::int as total_students,
          COUNT(CASE WHEN gender = 'MALE' THEN 1 END)::int as male_students,
          COUNT(CASE WHEN gender = 'FEMALE' THEN 1 END)::int as female_students,
        ...
        FROM students
      `;
            const stats = quickStatsResult[0];
            const totalStudents = Number(stats.total_students);
    ...
    static async getReports(_req, res) {
        try {
            ...
            const [totalStudentsResult, departmentsResult, studentsWithDetailsResult, documentsResult, academicYearsResult] = await Promise.all([
                prisma_1.prisma.$queryRaw `SELECT COUNT(*)::int as total FROM students`,
                prisma_1.prisma.$queryRaw `
          SELECT 
            d.id,
            d.name,
        ...
        `
            ...
            const totalStudents = Number(totalStudentsResult[0].total);
            const departments = departmentsResult;
            const studentsWithDetails = studentsWithDetailsResult;
            const yearlyData = academicYearsResult;
            console.log(`📄 Document types available: ${documentsResult.length}`);
            ...
            const reportsData = {
                ...
                trends: {
                    ...
                    graduationTrends: yearlyData.map(year => ({
                        year: year.academic_year,
                        count: Math.floor(Number(year.count) * 0.8)
                    })),
