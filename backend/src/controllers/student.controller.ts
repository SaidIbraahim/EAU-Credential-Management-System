...
const studentSchema = z.object({
  registrationId: z.string().min(3), // Temporarily relax validation for debugging
  certificateId: z.string().optional(), // Relax certificate ID validation too
...
export const StudentController = {
  async create(req: Request, res: Response): Promise<Response> {
    try {
      ...
      // Optimize duplicate check with single query using index
      const existingStudent = await prisma.student.findFirst({
        ...
        select: { id: true, registrationId: true } // Only select minimal fields
      });
      ...
      // Create student with minimal includes for faster response
  ...
  async getAllForValidation(_req: Request, res: Response): Promise<Response> {
    try {
  ...
  async bulkCreate(req: Request, res: Response): Promise<Response> {
    try {
      console.log('📥 Received bulk create request:', {
        ...
        firstStudent: req.body.students?.[0]
      });
